...
router.patch("/:id", verifyToken, isRoleAuthorizedMiddleware(["association", "family", "admin"]), verifyUser(), validate(patchSchema, "body"),withTryCatch(userController.patchUser)); // Route pour modifier un utilisateur par son ID

//* Routes accessibles uniquement aux admin, aux associations et aux familles d’accueil
router.delete("/:id", verifyToken, isRoleAuthorizedMiddleware(["admin", "association", "family"]), verifyUser(), withTryCatch(userController.deleteUser)); // Route pour supprimer un utilisateur par son ID
